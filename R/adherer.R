...
)
{
  ...
  )
  {
    ...
    return (list("Gaps"=Gaps, "CMA"=CMA, "event.info"=event.info));
  }
...
)
{
  ...
  {
    if( !suppress.warnings ) .report.ewms("I know how to do CMA sliding windows only for CMAs 1 to 10!\n", "error", "CMA_sliding_window", "AdhereR");
    return (NULL);
  }
  ...
  )
  {
    ...
    cat(paste("Line 9756", is.data.table(event.info.cma)))
  ...
  if( is.null(mg <- getMGs(ret.val)) )
  {
    ...
    cat(paste("Line 9828", is.data.table(tmp$event.info.cma)))
