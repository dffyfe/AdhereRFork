...
CMA0 <- function(data=NULL, # the data used to compute the CMA on
                 ...
                 js.tooltip.colname=NA, # the name of the column of the data to be shown as a tooltip for the plotted points
...
)
{
  ...
  structure(list("data"=data,
                 ...
                 "js.tooltip.colname"=js.tooltip.colname,
...
plot.CMA0 <- function(x,                                     # the CMA0 (or derived) object
                      ...
                      col.continuation="#FDB623", lty.continuation="dotted", lwd.continuation=1, # style of the contuniation lines connecting consecutive events
                      ...
                      html.plot.number=1,              # Is this the first plot in the HTML build, defaults to 1 if only plot in build
                      total.plots.in.html=1,           # How many plots in total are in the build (upto 10)?
                      use.custom.plots=FALSE,             # Use the plot names built into custom.plot.names parameter?
                      custom.plot.names = c("CustomPlot1", "CustomPlot2", "CustomPlot3", "CustomPlot4", "CustomPlot5", "CustomPlot6", "CustomPlot7", "CustomPlot8", "CustomPlot9", "CustomPlot10"), #Default custom plot names
...
)
{
  .plot.CMAs(x,
             ...
             html.plot.number=html.plot.number,
...
.compute.function <- function(fnc, # the function to compute
                              ...
                              js.tooltip.colname=NULL, # the name of the column of the data to be shown as a tooltip for the plotted evnets
                          ...
                          .workhorse.function)
{
  ...
  if( is.null(mg <- getMGs(ret.val)) )
  {
    ...
    # Adding Gaps/Leftover to output
    if(!is.null(tmp$Gaps))
    {
      setnames(tmp$Gaps, c(ID.colname, "Gaps"));
      ret.val[["Gaps"]] <- as.data.frame(tmp$Gaps);
    } else if (!is.null(tmp$Leftover))
    {
      setnames(tmp$Leftover, c(ID.colname, "Leftover"));
      ret.val[["Leftover"]] <- as.data.frame(tmp$Leftover);
    }
...
.plot.CMA1plus <- function(cma,                                   # the CMA1 (or derived) object
                           ...
                           generate.R.plot=TRUE,                  # generate standard (base R) plot for plotting within R?
                           html.plot.number=1,              # Is this the first plot in the HTML build, defaults to 1 if only plot in build?
                           total.plots.in.html=1,           # How many plots in total are in the build?
                           use.custom.plots=FALSE,             # Use the plot names built into custom.plot.names parameter?
                           custom.plot.names = c("CustomPlot1", "CustomPlot2", "CustomPlot3", "CustomPlot4", "CustomPlot5", "CustomPlot6", "CustomPlot7", "CustomPlot8", "CustomPlot9", "CustomPlot10"), #Default custom plot names
                           html.plot.colnames=NA, # Vector of column names to be displayed in an html table in the html output (if any)
                           logo.to.use = "AdhereR", # path to logo to use or if left as "AdhereR" adhereR logo
...
#'
#' Constructs a CMA (continuous multiple-interval measures of medication
#' availability/gaps) type 10 object.
#'
#' \code{CMA10} works off the assumption that all medication prescribed/dispensed within the observation
#' window (with the exception of the final event) is used in it's entirety. If the final event
#' extends past the final day of the OW it is trimmed to the number of days between the event and
#' the end of the OW. Similarly to CMA7, events prior to the start of the observation window are
#' considered for carryover purposes.
#'
#' CMA10 will also output the number of gap days (as calculated in cma7) and will be shown in
#' cma_sliding_window plot if no. of windows = 1.
#'
#' The formula is
#' \deqn{(days carryover from before OW + days supply within OW with final event trimmed if > OW) / (start to end of observation
#'  window)}
#'
#' @param data A \emph{\code{data.frame}} containing the events used to compute
#' the CMA. Must contain, at a minimum, the patient unique ID, the event date
...
#'             );
#' @export
CMA10 <- function( data=NULL, # the data used to compute the CMA on
                  # Important columns in the data
                  ID.colname=NA, # the name of the column containing the unique patient ID (NA = undefined)
                  js.tooltip.colname=NA, # the name of the column of the data to be shown as a tooltip for the plotted points
                  event.date.colname=NA, # the start date of the event in the date.format format (NA = undefined)
                  ...
                  arguments.that.should.not.be.defined=c("carryover.within.obs.window"=TRUE,
                                                         "carryover.into.obs.window"=TRUE,
                                                         "carry.only.for.same.medication"=FALSE,
                                                         "consider.dosage.change"=FALSE), # the list of argument names and values for which a warning should be thrown if passed to the function. cofsm & cdc added for CMA10 differing from CMA7
                  ...
)
{
  # The summary:
  if( is.na(summary) ) summary <- "The total available supply including carryover from before the OW. No assumptions are made on the final event within the observation window if it extends beyond the last day of the OW.";
  ...
  {
    # Auxiliary internal function: Compute the number of gap days for a given patient:
    .process.patient <- function(data4ID)
    {
      # Select the events within the observation window:
      s <- which(!data4ID$.EVENT.STARTS.BEFORE.OBS.WINDOW & !data4ID$.EVENT.STARTS.AFTER.OBS.WINDOW);
      ...
      if( slen < 1 || (data4ID$.DATE.as.Date[s1] > data4ID$.OBS.END.DATE[s1]) )
      {
        if ( sum(data4ID$.EVENT.STARTS.BEFORE.OBS.WINDOW)==0 )
        {
          # If there are no prior events, CMA10 gaps will be the duration of OW:
          return (as.numeric(difftime(data4ID$.OBS.END.DATE[1], data4ID$.OBS.START.DATE[1], units="days")));
        } else
        {
          # Select the last event that starts before the observation window begins but continue within the observation window:
          event.start.before.continue.in <- max(which(data4ID$.EVENT.STARTS.BEFORE.OBS.WINDOW));
          if( data4ID$gap.days[event.start.before.continue.in] <= as.numeric(data4ID$.OBS.END.DATE[n.events] - data4ID$.OBS.START.DATE[n.events]) )
          {
            # if there is an event that starts before and continues within OW and no events in OW then gaps on this event is equal to total gap days
            return (as.numeric(gap.days.column[event.start.before.continue.in]));
          } else
          {
            # If there is no event before that enter into the observation window, CMA10 Gaps is length of OW:
            return (as.numeric(difftime(data4ID$.OBS.END.DATE[1], data4ID$.OBS.START.DATE[1], units="days")));
          }
        }
      } else
      {
        ...
        {
          # if there is only one event, there are no "gap.days" to consider
          return (as.numeric(gap.days.obswin.last.event + gap.days.obswin.before.first.event));
        } else
        {
          # if there are more events, gap days are the ones from before, the ones for the last event, and the ones in between
          return (as.numeric(sum(gap.days.column[s[-slen]],na.rm=TRUE) + gap.days.obswin.last.event + gap.days.obswin.before.first.event));
        }
      }
    };
    ...
    .process.patient2 <- function(data4ID)
    {
      ...
      # cma10_durations <- ifelse(data4ID$.EVENT.STARTS.BEFORE.OBS.WINDOW & data4ID$.DATE.as.Date + event.duration.column >= data4ID$.OBS.START.DATE,
      #                           as.numeric(data4ID$.DATE.as.Date + event.duration.column - data4ID$.OBS.START.DATE),
      #                           ifelse(data4ID$.DATE.as.Date + event.duration.column >= data4ID$.OBS.END.DATE,
      #                                  as.numeric(data4ID$.OBS.END.DATE - data4ID$.DATE.as.Date), #this is wrong!!
      #                                  event.duration.column));

      # if ( slen < 1 ) {
      #   return (0.0);
      # } else {
      #   return (as.numeric(sum(cma10_durations[s], na.rm=TRUE))/
      #             as.numeric(difftime(data4ID$.OBS.END.DATE[s1], data4ID$.OBS.START.DATE[s1], units="days")))
      # };



      # # Original method begins here
      # # Select the events within the observation window:
      # s <- which(!data4ID$.EVENT.STARTS.BEFORE.OBS.WINDOW & !data4ID$.EVENT.STARTS.AFTER.OBS.WINDOW);
      # # Cache used things:
      # n.events <- nrow(data4ID); s1 <- s[1]; slen <- length(s); slast <- s[slen];
      # carry.over.column <- data4ID[, ".CARRY.OVER.FROM.BEFORE"];
      # event.duration.column <- data4ID[, get(event.duration.colname)]
      # event.start.before.continue.in <- max(which(data4ID$.EVENT.STARTS.BEFORE.OBS.WINDOW)); # Select the events that start before the observation window begins but continue within the observation window:
      #
      # # For less than one event or when the first event is on the last day of the observation window,
      # if( slen < 1 || (data4ID$.DATE.as.Date[s1] > data4ID$.OBS.END.DATE[s1]) )
      # {
      #   if ( sum(data4ID$.EVENT.STARTS.BEFORE.OBS.WINDOW)==0 )
      #   {
      #     # If there are no prior events, CMA10 does not make sense:
      #     return (NA_real_);
      #   } else
      #   {
      #     if(as.numeric(difftime(data4ID$.OBS.START.DATE[event.start.before.continue.in], data4ID$.DATE.as.Date[event.start.before.continue.in], units="days")) <= as.numeric(carry.over.column[event.start.before.continue.in] + event.duration.column[event.start.before.continue.in]))
      #     {
      #       # Otherwise, CMA10 is the gap of the last event entering the observation window minus any days between the end of observation window and date of next event or end of follow-up window, divided by the observation window:
      #       return ((as.numeric(carry.over.column[event.start.before.continue.in] + event.duration.column[event.start.before.continue.in]) - as.numeric(difftime(data4ID$.OBS.START.DATE[event.start.before.continue.in], data4ID$.DATE.as.Date[event.start.before.continue.in], units="days")))/
      #                 as.numeric(difftime(data4ID$.OBS.END.DATE[s1], data4ID$.OBS.START.DATE[s1], units="days")));
      #     } else
      #     {
      #       # If there is no event before that enter into the observation window, CMA10 is 0:
      #       return (0.0);
      #     }
      #   }
      # } else
      ...
      #   if ( sum(data4ID$.EVENT.STARTS.BEFORE.OBS.WINDOW)==0 )
      #   {
      #     return (as.numeric(sum(event.duration.column[s], na.rm=TRUE))/
      #               as.numeric(difftime(data4ID$.OBS.END.DATE[s1], data4ID$.OBS.START.DATE[s1], units="days")))
      #   } else
      #   {
      #     return ((max(0, as.numeric(carry.over.column[event.start.before.continue.in] + event.duration.column[event.start.before.continue.in]) - as.numeric(difftime(data4ID$.OBS.START.DATE[event.start.before.continue.in], data4ID$.DATE.as.Date[event.start.before.continue.in], units="days"))) +
      #                as.numeric(sum(event.duration.column[s], na.rm=TRUE)))/
      #               as.numeric(difftime(data4ID$.OBS.END.DATE[s1], data4ID$.OBS.START.DATE[s1], units="days")))
      #   }
      # }
    };
    ...
    event.info <- compute.event.int.gaps(data=as.data.frame(data),
                                         ...
                                         suppress.warnings=suppress.warnings,
                                         return.data.table=TRUE);
    if( is.null(event.info) ) return (list("CMA"=NA, "Gaps"=NULL, "event.info"=NULL));

    Gaps <- event.info[, .process.patient(.SD), by=ID.colname];
    CMA <- event.info[, .process.patient2(.SD), by=ID.colname];
    return (list("CMA"=CMA, "Gaps"=Gaps, "event.info"=event.info));
  }

  ret.val <- .cma.skeleton(data=data,
                           ret.val=ret.val,
                           cma.class.name=c("CMA10","CMA1"),
                           ID.colname=ID.colname,
                           js.tooltip.colname=js.tooltip.colname,
                           event.date.colname=event.date.colname,
                           event.duration.colname=event.duration.colname,
                           ...
                           carryover.within.obs.window=TRUE,
                           carryover.into.obs.window=TRUE, # if TRUE consider the carry-over from before the starting date of the observation window
                           carry.only.for.same.medication=FALSE, #a change from CMA7
                           consider.dosage.change=FALSE, #a change from CMA7
                           followup.window.start=followup.window.start,
                           followup.window.start.unit=followup.window.start.unit,
                           followup.window.duration=followup.window.duration,
                           followup.window.duration.unit=followup.window.duration.unit,
...
#' @rdname print.CMA0
#' @export
print.CMA10 <- function(...) print.CMA0(...)

#' @rdname plot.CMA1
#' @export
plot.CMA10 <- function(...) .plot.CMA1plus(...)
...
#' function.
#' @param ... other possible parameters
#' @return An \code{S3} object of class \code{CMA11} (derived from \code{CMA0})
#' with the following fields:
#' \itemize{
#'  \item \code{data} The actual event data, as given by the \code{data}
#'  parameter.
...
#'               );
#' @export
CMA11 <- function( data=NULL, # the data used to compute the CMA on
                  # Important columns in the data
                  ID.colname=NA, # the name of the column containing the unique patient ID (NA = undefined)
                  js.tooltip.colname=NA, # the name of the column of the data to be shown as a tooltip for the plotted points
                  event.date.colname=NA, # the start date of the event in the date.format format (NA = undefined)
...
)
{
  ...
  )
  {
    ...
    # Auxiliary internal function: Compute leftover supply at end of OW for a given patient:
    .process.patient2 <- function(data4ID)
    {
      # Select the events within the observation window:
      s <- which(!data4ID$.EVENT.STARTS.BEFORE.OBS.WINDOW & !data4ID$.EVENT.STARTS.AFTER.OBS.WINDOW);
...
#' @rdname print.CMA0
#' @export
print.CMA11 <- function(...) print.CMA0(...)

#' @rdname plot.CMA1
#' @export
plot.CMA11 <- function(...) .plot.CMA1plus(...)
...
)
{
  ...
  } else if( CMA.to.apply %in% c("CMA5", "CMA6") )
  {
    ...
    if( is.na(carry.only.for.same.medication) ) carry.only.for.same.medication <- def.vals[["carry.only.for.same.medication"]]; # use the default value from CMA
    if( is.na(consider.dosage.change) ) consider.dosage.change <- def.vals[["consider.dosage.change"]]; # use the default value from CMA
  } else if( CMA.to.apply %in% c("CMA7", "CMA8", "CMA9", "CMA11") )
  {
    carryover.into.obs.window <- carryover.within.obs.window <- TRUE;
    if( is.na(carry.only.for.same.medication) ) carry.only.for.same.medication <- def.vals[["carry.only.for.same.medication"]]; # use the default value from CMA
    if( is.na(consider.dosage.change) ) consider.dosage.change <- def.vals[["consider.dosage.change"]]; # use the default value from CMA
  ...
  )
  {
    ...
    .process.patient <- function(data4ID)
    {
      ...
      } else if (!is.null(cma$Leftover))
      {
        wnd.info <- merge(wnd.info, cma$Leftover, by=".WND.ID", all=TRUE);
      }
      ...
      if (!is.null(cma$Gaps))
      {
        setnames(wnd.info, c("window.ID", "window.start", "window.end", "Gaps", "CMA", "CMA.to.apply"));
      } else if (!is.null(cma$Leftover))
      {
        setnames(wnd.info, c("window.ID", "window.start", "window.end", "Leftover", "CMA", "CMA.to.apply"));
      } else
      {
