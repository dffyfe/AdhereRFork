...
)
{
  ...
  r <- list(# The initial comment (if any):
    ...
    if(!is.na(tooltip) && is.na(js_tooltip)) c('>',' <title>', tooltip, '</title>') else '></rect>', # the tooltip title must be first child
...
.SVG.lines <- function(x, y,  # the coordinates of the points (at least 2)
                       ...
                       id=NA, class=NA, comment=NA, tooltip=NA, js_tooltip=NA,  # ID, comment, tooltip and js tooltip
                       newline=TRUE, # should a newline be added at the end?
                       return_string=FALSE, # return a singe string or a vector of strings to be concatenated later?
...
)
{
  ...
  if(connected)
  {
    ...
    r <- list(# The initial comment (if any):
      ...
      # Close the element (and add optional tooltip):
      if(!is.na(tooltip) && is.na(js_tooltip)) c('>',' <title>', tooltip, '</title>', '</polyline>') else '></polyline>', # the tooltip title must be first child
    ...
    r <- list(# The initial comment (if any):
      ...
      lapply(seq(1,length(x),by=2), function(i)
      {
        ...
        list('<line ',
          ...
          # Close the element (and add optional tooltip):
          if(!is.na(tooltip) && is.na(js_tooltip)) c('>',' <title>', tooltip, '</title>', '</line>') else '></line>', # the tooltip title must be first child
  ...
  r <-  list(# The initial comment (if any):
    ...
    lapply(seq_along(x), function(i)
    {
      list(# The element:
        ...
        # The id class and tooltip (if any):
        if(!is.na(id)) c('id="',id,'" '),
        if(!is.na(class)) c('class="',class,'" '),
        if(!is.na(js_tooltip)) c('onmousemove="adh_svg.show_tooltip(evt, \'', js_tooltip, '\');" onmouseout="adh_svg.hide_tooltip();"'),
        '>',
        ...
        # Reuse the predefined symbol:C:\Users\David\Desktop\NHS WFH\MAVIS\AdhereR-plot with select with javascript to create select.html
...
.plot.CMAs <- function(cma,                                   # the CMA_per_episode or CMA_sliding_window (or derived) object
                       ...
                       html.plot.colnames = NA,         # What columns from cma$data are to be displayed in the html table in the data tab
...
)
{
  ...
  # Patient IDs, medical class and jstooltip col better be characters:
  ...
  {
    cma$data[, cma$js.tooltip.colname] <- as.character(cma$data[, cma$js.tooltip.colname]);
  }
  ...
  # if(col.cats == "Rainbow") {
  #   col.cats <- rainbow;
  # } else if(col.cats == "ColorBlind" && length(categories) < 12) {
  #   col.cats <- .cb.colors;
  # } else {
  ...
  if( is.na(categories[1]) || categories[1] == "000000A" ) #000000A is in place of NA which results in an error in sliding window designed to trigger na.col colour for scaled colour chart in STUMedView
  {
  ...
  if( .do.SVG )
  {
    ...
    categories.to.classes <- paste0("med-class-Graph",html.plot.number,"-",1:length(categories)); names(categories.to.classes) <- categories;
            ...
            if( .do.SVG ) # SVG:
            {
              ...
              svg.str[[length(svg.str)+1]] <-
                ...
                .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[s.events[1],".X.ROW.START"], y=.last.cma.plot.info$SVG$cma$data[s.events[1],".Y.ROW.START"],
                          ...
                          height=.scale.height.to.SVG.plot(vspace.needed.events),
                          stroke="none", fill=observation.window.col, fill_opacity=observation.window.opacity,
                          class="ow-real", comment="The 'real' Observation Window", js_tooltip="'Real' Observation Window");
            }
          }
        }
      }
    ...
    if( .do.SVG ) # SVG:
    {
      ...
      svg.str[[length(svg.str)+1]] <-
        ...
        .SVG.points(x=.last.cma.plot.info$SVG$cma$data[i,".X.START"], y=.last.cma.plot.info$SVG$cma$data[i,".Y.START"],
                    pch=pch.start.event, col=col, cex=cex,
                    class=paste0("event-start",if(!is.na(med.class.svg)) paste0("-",med.class.svg)),
                    js_tooltip=js.tooltip.svg, suppress.warnings=suppress.warnings);
      svg.str[[length(svg.str)+1]] <-
        # The end of the event:
        .SVG.points(x=.last.cma.plot.info$SVG$cma$data[i,".X.END"], y=.last.cma.plot.info$SVG$cma$data[i,".Y.END"],
                    pch=pch.end.event, col=col, cex=cex,
      ...
      if( .do.SVG ) # SVG:
      {
        ...
        svg.str[[length(svg.str)+1]] <-
          ...
          .SVG.text(x=.last.cma.plot.info$SVG$cma$data[i,".X.DOSE"], y=.last.cma.plot.info$SVG$cma$data[i,".Y.DOSE"],
                    ...
                    js_tooltip=med.class.svg.name, suppress.warnings=suppress.warnings);
      }
    }
        ...
        svg.str[[length(svg.str)+1]] <-
          ...
          .SVG.lines(x=c(.last.cma.plot.info$SVG$cma$data[i,".X.CNT.START"],
                     ...
                     js_tooltip= if(.last.cma.plot.info$SVG$cma$computed.CMA %in% c("CMA7", "CMA10", "CMA11") && is.cma.TS.or.SW)
                                  {
                                    if(is.na(.last.cma.plot.info$SVG$cma$event.info.cma[i, "gap.days"]))
                                    {
                                      NA;
                                    } else
                                    {
                                      if(is.na(.last.cma.plot.info$SVG$cma$event.info.cma[i+1, ".CARRY.OVER.FROM.BEFORE"]))
                                      {
                                        paste0(sprintf("%.f",.last.cma.plot.info$SVG$cma$event.info.cma[i, "gap.days"]), " day(s) not covered.")
                                      } else
                                      {
                                  ...
                                  {
                                    if(is.na(.last.cma.plot.info$SVG$cma$event.info[i, "gap.days"]))
                                    {
                                      NA;
                                    } else
                                    {
                                      if(is.na(.last.cma.plot.info$SVG$cma$event.info[i+1, ".CARRY.OVER.FROM.BEFORE"]))
                                      {
                                        paste0(sprintf("%.f",.last.cma.plot.info$SVG$cma$event.info[i, "gap.days"]), " day(s) not covered.")
                                      } else
                                      {
      ...
      if( .do.SVG ) # SVG:
      {
        svg.str[[length(svg.str)+1]] <- list(
          ...
          .SVG.rect(x=.scale.x.to.SVG.plot(.rescale.xcoord.for.CMA.plot(0.0)),
                    ...
                    class="cma-drawing-area-bkg", js_tooltip="CMA estimate"),
  ...
  if( .do.SVG ) # SVG:
  {
    svg.str[[length(svg.str)+1]] <- list(
      ...
      .SVG.text(x=dims.chr.axis, y=dims.total.height/2,
                ...
                class="axis-name-y", comment="The y-axis label", js_tooltip="Y axis: patients, events and (possibly) CMA estimates", suppress.warnings=suppress.warnings),
      ...
      .SVG.text(x=(dims.plot.x + dims.total.width)/2, y=(dims.total.height - dims.chr.axis),
                ...
                class="axis-name-x", comment="The x-axis label", js_tooltip="X axis: the events ordered in time (from left to right)", suppress.warnings=suppress.warnings)
    );
    ...
    if( !is.null(export.formats) )
    {
      ...
      if( "html" %in% export.formats )
      {
        ...
        if(logo.to.use != "AdhereR" && file.exists(paste0(export.formats.directory, '/', logo.to.use)) && html.plot.number==1) {
         ...
         # Add the plot names json:
        if(html.plot.number==1 & use.custom.plots==FALSE) { # js object for generic plot names
          js.template <- c(js.template,
                            '// Plotnames or assigned numbers',
                            'adh_svg["plot_names"] = {\n',
                            paste0('  "', paste0('Graph ', 1:total.plots.in.html), '" : "', 1:total.plots.in.html,'"',collapse=',\n'),
                            '\n};\n')
        } else if (html.plot.number==1 & use.custom.plots==TRUE) { #js object for defined plot names
           js.template <- c(js.template,
                            '// Plotnames or assigned numbers',
                            'adh_svg["plot_names"] = {\n',
        ...
        if( html.plot.number == 1) #add the js/css for first/only block
        {
          ...
          # if(total.plots.in.html !=1 & use.custom.plots==FALSE) {
          #   html.template <- sub('//<!--Plot Names Placeholder-->',
          #                      paste0('// Plotnames or assigned numbers\n',
          #                      'adh_svg["plot_names"] = {\n',
          #                      paste0('  "', paste0('Graph ', 1:total.plots.in.html), '" : "', 1:total.plots.in.html,'"',collapse=',\n'),
          #                      '\n};\n'),
          #                      html.template, fixed=TRUE);
          # } else if (total.plots.in.html !=1 & use.custom.plots==TRUE) {
          #   html.template <- sub('<!--Plot Names Placeholder-->',
          #                      paste0('// Plotnames or assigned numbers\n',
          #                      'adh_svg["plot_names"] = {\n',
          #                      paste0('  "', custom.plot.names, '" : "', 1:length(custom.plot.names),'"',collapse=',\n'),
        ...
        if (all(!is.na(html.plot.colnames)) && all(html.plot.colnames %in% colnames(cma$data))) {
          ...
          html.template <- sub(paste0('<!--Table ', html.plot.number, ' placeholder-->'), # sub the table for this plot number (if requested)
                               htmltools::tags$table(id=paste0("table", html.plot.number),
                                 htmltools::tags$thead(htmltools::tags$tr(lapply(colnames(cma$data[html.plot.colnames]), function(x) htmltools::tags$th(x)))),
                                 htmltools::tags$tbody(
                                   apply(cma$data[html.plot.colnames],1, function(x) htmltools::tags$tr(lapply(x, function(y) htmltools::tags$td(y))))
                                 )),
                               html.template, fixed=TRUE);
        }
        ...
        svg.str.embedded <- c('<svg class="adherence_plot index', html.plot.number+1, '" ', # add class and (possibly) the dimensions to the <svg> tag
                              'style="z-index: 1;" ', # add z-index to place refresh zoom button on top of svg
                              if( FALSE ) 'height="600" ', # height (if defined)
                              if( TRUE ) paste0('width="',(dims.plot.width / dims.chr.std),'" '), # width in "standard" charters
                              svg.str[-1]);
        if ( html.plot.number == 1) # sub the full svg in to placeholder for first/only block or replace placeholder if html.plot.number>1
        { html.template <- sub('<object id="adherence_plot" data="PATH-TO-IMAGE" type="image/svg+xml">Please use a modern browser!</object>',
          ...
          html.template <- sub(paste0('<!--Plot Number ', html.plot.number, ' placeholder-->'),
                               paste0(paste0(svg.str.embedded, collapse=""), "\n"),
                               html.template, fixed=TRUE);
        }
